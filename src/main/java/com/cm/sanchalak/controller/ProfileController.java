...
public class ProfileController {
     ...
     * GET /api/me/attendance/history?studentId={id}&startDate={date}&endDate={date} (for parents)
     * Auto-resolves studentId for STUDENT role, validates linkage for PARENT role
     */
    ...
    public ResponseEntity<ApiResult<List<AttendanceRecordDto>>> getAttendanceHistory(
        ...
        List<AttendanceRecordDto> history = attendanceService.getStudentAttendanceHistory(
            resolvedStudentId, startDate, endDate);
            ...
            .map(term -> {
                ...
                List<ResultsDto.SubjectScoreDto> subjectScores = term.getSubjects().stream()
                    .map(subject -> ResultsDto.SubjectScoreDto.builder()
                        ...
                        .obtainedMarks(subject.getMarksObtained() != null ? subject.getMarksObtained() : 0.0)
                        .build())
