...
public class NotificationScheduler {
    ...
    public void sendFeeDueReminders() {
        ...
        try {
            ...
            // List<Fee> upcomingFees = feeRepository.findDueFeesInDateRange(today, dueDate);
            // 
            // for (Fee fee : upcomingFees) {
            //     Student student = fee.getStudent();
            //     List<ParentStudentLink> links = parentStudentLinkRepository.findActiveByStudentId(student.getId());
            //     
    ...
    public void monitorBusProximity() {
        log.debug("Checking bus proximity for alerts");
        ...
        try {
            ...
            // List<Trip> activeTrips = tripRepository.findByDateAndStatus(LocalDate.now(), "IN_PROGRESS");
            // 
            // for (Trip trip : activeTrips) {
            //     LocationPing latestLocation = locationPingRepository.findLatestByVehicleId(trip.getVehicle().getId());
            //     if (latestLocation == null) continue;
            //     
            //     List<Stop> upcomingStops = stopRepository.findByRouteIdOrderByStopOrder(trip.getRoute().getId());
            //     
            //     for (Stop stop : upcomingStops) {
            //         double distanceKm = calculateDistance(latestLocation.getLatitude(), latestLocation.getLongitude(),
            //                                               stop.getLatitude(), stop.getLongitude());
            //         
            //         if (distanceKm <= 2.0) {
            //             // Get students at this stop
            //             List<StudentTransportAssignment> assignments = assignmentRepository.findActiveByStopId(stop.getId());
            //             
