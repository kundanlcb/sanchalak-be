...
public class NoticeManagementIntegrationTest {
    ...
    void setUp() {
        ...
        // If the controller uses @CurrentUser to fetch User object from DB, we need to insert it.
        ...
        // However, @WithMockUser provides a principal, but it doesn't automatically insert into DB.
        // We'll rely on the fact that we can't easily injection a specific UUID with @WithMockUser unless we use a custom annotation.
        // Actually, @CurrentUser usually resolves UserPrincipal. If UserPrincipal contains ID, we are good.
    ...
    void testCreateNotice() throws Exception {
        ...
        // Strategy: We will create a user in DB, and then specific that user's details if we could.
        // But with Spring Security Test, it's easier if we just mock the Service or specific the Principal.
        // A better approach for integration tests is to use a setup method that creates a user and token, then passes the token.
        // But for "quick" verification, let's try to see if we can get away with mocking the Service? 
        ...
        // Actually, to make this robust, we should use a custom @WithUserDetails or setup the SecurityContext manually.
